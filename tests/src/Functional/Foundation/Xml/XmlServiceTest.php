...
use Derafu\Lib\Core\Foundation\Xml\Worker\Decoder;
use Derafu\Lib\Core\Foundation\Xml\Worker\Encoder;
use Derafu\Lib\Core\Foundation\Xml\Worker\Validator;
...
class XmlServiceTest extends TestCase
{
    ...
    private static function dataProvider(string $testName): array
    {
        if (!isset(self::$testCases)) {
            $testCasesFile = self::getFixturesPath(
                'foundation/xml/encode_and_decode.php'
            );
    ...
    public function testValidateSchemaSuccess(): void
    {
        ...
        try {
            $this->xmlService->validateSchema($xml, $schemaPath);
